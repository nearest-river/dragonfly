...
#[diagnostic::on_unimplemented(message = "`{Self}` doesn't implement `DynSend`. Add it to `rustc_data_structures::marker` or use `IntoDynSyncSend` if it's already `Send`")]
...
already_send!(
    [std::sync::atomic::AtomicBool][std::sync::atomic::AtomicUsize][std::sync::atomic::AtomicU8]
        ...
        [std::io::Error][std::fs::File][std::panic::Location<'_>][dragonfly_arena::DroplessArena]
...
impl_dyn_send!(
    ...
    [dragonfly_arena::TypedArena<T> where T: DynSend]
